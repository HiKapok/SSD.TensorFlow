...
def slim_get_split(file_pattern='{}_????'):
    ...
    [org_image, shape, glabels_raw, gbboxes_raw, isdifficult] = provider.get(['image', 'shape',
                                                                         ...
                                                                         'object/bbox',
                                                                         'object/difficult'])
    image, glabels, gbboxes = ssd_preprocessing.preprocess_image(org_image, glabels_raw, gbboxes_raw, tf.cast(shape[:-1], tf.int32), is_training=True, data_format='channels_first')
