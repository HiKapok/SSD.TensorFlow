...
def ssd_random_sample_patch_wrapper(image, labels, bboxes):
  with tf.name_scope('ssd_random_sample_patch_wrapper'):
    ...
    def check_bboxes(bboxes):
      ...
      return tf.logical_and(tf.logical_and(areas < 0.9, areas > 0.01),
                            tf.logical_and((bboxes[:, 3] - bboxes[:, 1]) > 0.03, (bboxes[:, 2] - bboxes[:, 0]) > 0.03))
    index = 0
    max_attempt = 3
...
def preprocess_for_train(image, labels, bboxes, out_shape, data_format='channels_first', scope='ssd_preprocessing_train'):
  ...
  with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
    ...
    random_sample_image, labels, bboxes = ssd_random_sample_patch_wrapper(image, labels, bboxes)
    # image, bboxes = tf.cond(tf.random_uniform([1], minval=0., maxval=1., dtype=tf.float32)[0] < 0.25,
    #                     lambda: (image, bboxes),
    #                     lambda: ssd_random_expand(image, bboxes, tf.random_uniform([1], minval=2, maxval=4, dtype=tf.int32)[0]))
    ...
    distort_image = apply_with_random_selector(
